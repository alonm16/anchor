...
for ds_name in [ 'dilemma']:
    path = f'results/mp/{model_type}/{ds_name}/{sorting}/'
    
    aopc = AOPC(path, tokenizer)
    aopc.compare_all(only = ['aggregation'], agg_params = (aggregations, alphas, legends))
...
for ds_name in ['corona', 'toy-spam', 'home-spam', 'dilemma']:
    ...
    aopc = AOPC(path, tokenizer)
    aopc.compare_all(only = ['aopc time aggregation'], agg_params = (aggregations, alphas, legends))
