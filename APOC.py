...
class APOCV3:
    def __init__(self, model, tokenizer, sentences, labels, pos_tokens, neg_tokens, title, num_removes = 25):
        ...
        self.unmasker = pipeline('fill-mask', model='distilbert-base-uncased')
        self.softmax = torch.nn.Softmax()
        self.formula_type = 'v3'
        self.tokens_method = self._remove_tokens
        self.title = title
        self.num_removes = num_removes
        
        self.pos_sentences, self.pos_tokens, self.pos_shuffled_tokens, self.pos_reversed_tokens = APOCV3.prepare_apoc(tokenizer, pos_tokens, sentences, labels, 1)
        self.neg_sentences, self.neg_tokens, self.neg_shuffled_tokens, self.neg_reversed_tokens = APOCV3.prepare_apoc(tokenizer, neg_tokens, sentences, labels, 0)
    ...
    def _apoc_formula(self, orig_predictions, predictions_arr, k, tokens_idxs_count):
        if k == 0:
            return 0
        # no use summing from 0 because it's 0
        return sum([sum(orig_predictions - predictions_arr[i])/tokens_idxs_count[i] for i in range(1, k+1)])/(k+1)
    
    def _apoc_formula_v2(self, orig_predictions, predictions_arr, k, tokens_idxs_count):
        if k == 0:
            return 0
        N = tokens_idxs_count[k]
        return sum(orig_predictions - predictions_arr[k])/N

    def _calc_apoc(self, predictions_arr, tokens_idxs_count):
        predictions_arr = np.array(predictions_arr)
        orig_predictions = np.array(predictions_arr[0])
        formula_fn = self._apoc_formula if self.formula_type == 'v1' else self._apoc_formula_v2
        return [formula_fn(orig_predictions, predictions_arr, k, tokens_idxs_count) for k in range(len(predictions_arr))]

    def get_tokens_indices(self, sentences):
        tokens_idxs = defaultdict(set)   
        for i, s in enumerate(sentences):
            for w in s:
                tokens_idxs[w].add(i)
        return tokens_idxs
    
    def _apoc_global(self, tokens_arr, sentences, labels):
        removed_sentences_arr = []
        removed_sentences = sentences
        tokens_idxs = self.get_tokens_indices(sentences)
        tokens_idxs_count = []
        cur_set = set()
        
        for i in range(self.num_removes+1):
            # for every sentence the same tokens need to be erased
            cur_set = cur_set.union(tokens_idxs[tokens_arr[0][i]])
            tokens_idxs_count.append(len(cur_set))
            removed_sentences = self.tokens_method(i, tokens_arr, removed_sentences)
            removed_sentences_arr.append(removed_sentences) 
           
        predictions_arr = self._apoc_predictions(removed_sentences_arr, labels)
        apoc_scores = self._calc_apoc(predictions_arr, tokens_idxs_count)
        return apoc_scores
    ...
    def compare_apocs(model, tokenizer, sentences, labels, pos_tokens_arr, neg_tokens_arr, legends): 
        ...
        neg_scores = []
        for i in range(len(legends)):
            apoc = APOCV3(model, tokenizer, sentences, labels, pos_tokens_arr[i], neg_tokens_arr[i], "") 
            pos_scores.append(apoc._apoc_global(apoc.pos_tokens, apoc.pos_sentences, [1]*len(apoc.pos_sentences)))
            neg_scores.append(apoc._apoc_global(apoc.neg_tokens, apoc.neg_sentences, [0]*len(apoc.neg_sentences)))
